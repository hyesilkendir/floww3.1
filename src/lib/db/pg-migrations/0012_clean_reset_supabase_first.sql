...
-- Migration: 0012_clean_reset_supabase_first
-- This creates a completely fresh, simple schema that uses ONLY Supabase Auth
...
DROP TABLE IF EXISTS users CASCADE; -- Remove custom users table - use auth.users
...
-- Categories (per user - using auth.users)
...
-- Clients (per user - using auth.users)
...
-- Employees (per user - using auth.users)
...
-- Transactions (per user - using auth.users)
...
-- ==========================================
-- 6️⃣ ROW LEVEL SECURITY (Simple & Secure)
-- ==========================================
